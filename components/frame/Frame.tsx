...
export function Frame({
  ...
  return (
    ...
    >
      ...
      <span className="absolute left-5 top-5 inline-flex items-center gap-1 text-micro uppercase text-text-secondary">
        {label}
        <span aria-hidden="true">↗</span>
      </span>

      <div className="absolute inset-x-0 bottom-0 flex flex-col p-5">
        <h3 className="font-display text-d-1 text-text-primary">{title}</h3>
        {meta ? (
          <p className="mt-2 font-mono text-nano uppercase tabular-nums text-text-metadata">
            {meta}
          </p>
        ) : null}
        <span className="mt-4 h-px w-full bg-line-hairline transition-colors duration-fast ease-out group-hover:bg-patina-500 group-focus-visible:bg-patina-500" />
        {cta ? (
          <span className="mt-3 text-micro uppercase text-text-secondary">
            {cta}
          </span>
        ) : null}
      </div>
